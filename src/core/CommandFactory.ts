import type { CommandFactory as ICommandFactory, SerializedCommand, Command } from "../types/api";
export class DefaultCommandFactory<TState> implements ICommandFactory<TState> {
  private registry = new Map<string, (data: SerializedCommand) => Command<TState>>();
  ...
  create(data: SerializedCommand): Command<TState> {
    ...
    if (!b) throw new Error(`No builder registered for command type: ${data.type}`);
    return b(data);
  }
}
