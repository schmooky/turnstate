...
class SpendResource extends BaseCommand<MyState> {
  ...
  constructor(playerId: string, amount: number) {
    super({ playerId, type: "SpendResource" });
    this.amount = amount;
  }
  validate(s: MyState) {
    const have = s.resources.get(this.playerId) ?? 0;
    if (this.playerId !== s.turn)
      return { valid: false, errors: ["Not your turn"] };
    if (have < this.amount)
      return { valid: false, errors: ["Insufficient resources"] };
  ...
  execute(s: MyState) {
    ...
    return {
      success: true,
      state: { ...s, resources: next },
      sideEffects: [
        { type: "broadcast", data: { spent: this.amount, by: this.playerId } },
      ],
    };
  }
  undo(s: MyState) {
  ...
  validators: [
    {
      name: "TurnValidator",
      validate(cmd, s) {
        return cmd.type === "SpendResource" && (cmd as any).playerId !== s.turn
...
const rehydrated = new StateMachine<MyState>(
  { turn: "p1", players: [], resources: new Map() },
  { serializer: new JsonSerializer<MyState>(), commandFactory: factory },
);
