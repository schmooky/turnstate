...
class SetV extends BaseCommand<S> {
  constructor(playerId: string, private v: number) { super({ playerId, type: "SetV" }); }
  execute(_s: S) { return { success: true, state: { v: this.v }, sideEffects: [{ type: "notify", data: { v: this.v } }] }; }
  undo(_s: S) { return { success: true, state: { v: 0 } }; }
...
describe("events", () => {
  it("emits events with previous/next/command/sideEffects", () => {
    ...
    sm.subscribe((e) => { captured = e.next.v; });
